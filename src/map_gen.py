...
class MapGenerator:
    ...
    def generate_map(self, seed):
        random.seed(seed)
        generated_map = [
            [0 for y in range(self.tiles_h * self.map_h_tiles)]
            for x in range(self.tiles_w * self.map_w_tiles)
        ]
        last_tile = Tile(0, 0)
            ...
            for tiles_x in range(self.map_w_tiles):
                ...
                if tiles_x == 0 and tiles_y == 0:  # starting tile
                    ...
                    if tiles_x == 0:  # start of new line
                        current_tile = random.choice(
                            self.connections["N"][
                                last_start_of_line_tile.get_connection("S")
                            ]
                        )
                    else:
                        current_tile = random.choice(
                            self.connections["W"][last_tile.get_connection("E")]
                        )
                ...
                for y in range(current_tile.h):
                    for x in range(current_tile.w):
                        ...
                        generated_map[pos_x_in_map][pos_y_in_map] = current_tile.mat[x][
                            y
                        ]
                ...
                if (
                    tiles_x == 0
                ):  # this tile started a line, so it will be used for the next one
